...
task InitialPrivateBuild -depends StopSystem, Clean, Compile


task Nuget -depends SetReleaseBuild, Clean, Compile, PackageYayCore, PackageYayMvc
...
task PackageYayMvc -depends SetReleaseBuild, Compile {
    delete_directory $temp_package_dir
	
    #dlls
    copy_files "$source_dir\Yay.Enumerations.MVC\bin\Release\Yay.Enumerations.MVC.dll" "$temp_package_dir\lib"
    copy_files "$source_dir\Yay.Enumerations.MVC\bin\Release\Yay.Enumerations.MVC.pdb" "$temp_package_dir\lib"
    copy_files "$source_dir\Yay.Enumerations.MVC\Views\Shared\EditorTemplates" "$temp_package_dir\Content\Views\Shared\EditorTemplates"
    copy_files "$source_dir\Yay.Enumerations.MVC\Views\Shared\DisplayTemplates" "$temp_package_dir\Content\Views\Shared\DisplayTemplates"
    
	#nuget spec (and any other needed nuget thing)
	copy_files "$base_dir\nuget\Yay.Enumerations.MVC\" "$temp_package_dir"
	
	$nuspec_file = "$temp_package_dir\Yay.Enumerations.MVC.nuspec"
	package-for-nuget $nuspec_file $package_dir
}
...
task StartUi -depends StopSystem, StartSystem {
}
...
function deploy-database($action,$server,$db_name,$scripts_dir) {
    exec { .\lib\tarantino\DatabaseDeployer.exe $action $server $db_name $scripts_dir }
}
